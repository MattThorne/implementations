...
operation TestingSquareMultiply(): Unit{
    ...
    Message($"Final Result is: {accurate}");

}


operation SquareAndMultiply(ai:Int,mi:Int,ji:Int): Int{

    using ((a,m,j,result)=(Qubit[4],Qubit[4],Qubit[8],Qubit[4])){
        let ain = IntAsBoolArray(ai,4);
        let min = IntAsBoolArray(mi,4);
        let jin = IntAsBoolArray(ji,8);
        for (i in 0..(Length(ain)-1)){
            if (ain[i] == true){X(a[i]);}
            if (min[i] == true){X(m[i]);}
        }
        for (i in 0..(Length(jin)-1)){
            if (jin[i] == true){X(j[i]);}
        }
        ...
        let res = MeasureInteger(LittleEndian(result));
        ResetAll(a + m + j + result);
        return res;
    }
}
