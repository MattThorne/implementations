...
operation Testing_with_Toffoli(aI:BigInt,mI:BigInt,numBits:Int):Int{
    let aArr = BigIntAsBoolArray(aI);
    let mArr = BigIntAsBoolArray(mI);
    using ((a,m,t) = (Qubit[numBits],Qubit[numBits],Qubit[numBits])){
...
let numQubits = 3;
using ((a,m,t) = (Qubit[numQubits],Qubit[numQubits],Qubit[numQubits])){
...
operation ModularSquaring(a:Qubit[],m:Qubit[],Ts:Qubit[]) : Unit is Adj + Ctl{
    let num = Length(a);
    using ((a2,a2Pad,zC) = (Qubit[num],Qubit[num],Qubit())){
        ApplyToEachCA(X,m);
        Controlled X(m,zC);
        ApplyToEachCA(X,m);
        X(zC);
        
        Controlled SquareI([zC],(LittleEndian(a),LittleEndian(a2 + a2Pad)));

        using ((anc,mPad) = (Qubit[num*2],Qubit[num])){

            Controlled DivideI([zC],(LittleEndian(a2 + a2Pad),LittleEndian(m + mPad),LittleEndian(anc)));

            Controlled AddI([zC],(LittleEndian(a2),LittleEndian(Ts)));

            Controlled Adjoint DivideI([zC],(LittleEndian(a2 + a2Pad),LittleEndian(m + mPad),LittleEndian(anc)));

        } 
        Controlled Adjoint  SquareI([zC],(LittleEndian(a),LittleEndian(a2 + a2Pad)));
